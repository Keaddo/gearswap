...
-- Added rules for PetOnly idle Axe swaps (TP, PDT, MDT, Idle).
...
-- Random Lockstyle generator.
    if RandomLockstyleGenerator == 'true' then
        local random_lockstyle_list = {1,2,3,4,7,8,10,13,14,15,16}
...
function init_gear_sets()
    ...
    Reward_Axe = {name="Kumbhakarna", augments={'Pet: Mag. Evasion+20','MND+16',}}
    Reward_Axe2 = "Odium"
    Reward_hands = {name="Taeon Gloves", augments={'Accuracy+18 Attack+18','"Dual Wield"+5','MND+9',}}
    Reward_back = {name="Artio's Mantle", augments={'MND+20','Mag. Acc+20 /Mag. Dmg.+20','"Fast Cast"+10',}}

    Pet_PDT_head = "Anwig Salade"
    Pet_PDT_body = "Emicho Haubert +1"
    ...
    Pet_Regen_body = "Emicho Haubert +1"
    ...
    Ready_Acc_legs = {name="Valor. Hose", augments={'Pet: Accuracy+30 Pet: Rng. Acc.+30','Pet: DEX+6',}}
    ...
    sets.PDTAxeShield = {main=Pet_PDT_AxeMain,sub="Beatific Shield +1"}
    sets.MDTAxeShield = {main=Pet_MDT_AxeMain,sub="Beatific Shield +1"}
    sets.TPAxeShield = {main=Pet_TP_AxeMain,sub="Beatific Shield +1"}
    ...
    sets.PDTAxes = {main=Pet_PDT_AxeMain,sub=Pet_PDT_AxeSub}
    sets.MDTAxes = {main=Pet_MDT_AxeMain,sub=Pet_MDT_AxeSub}
    sets.TPAxes = {main=Pet_TP_AxeMain,sub=Pet_TP_AxeSub}
    ...
    sets.defense.PDT = {
        ...
        back="Solemnity Cape",waist="Flume Belt +1",legs="Arjuna Breeches",feet="Amm Greaves"}
    ...
    sets.precast.WS['Ruinator'] = {ammo="Cheruski Needle",
        ...
        back="Grounded Mantle +1",waist="Fotia Belt",legs="Obatala Subligar",feet="Emicho Gambieras +1"}
    ...
    sets.engaged = {ammo="Paeapua",
        ...
        back="Grounded Mantle +1",waist="Windbuffet Belt +1",legs="Obatala Subligar",feet="Emicho Gambieras +1"}
        ...
        if pet.status == "Engaged" then
            if player.sub_job == 'NIN' or player.sub_job == 'DNC' then
                ...
                elseif state.DefenseMode.value == "Magical" and state.MagicalDefenseMode.value == "PetMDT" then
                    ...
                    idleSet = set_combine(idleSet, sets.TPAxes)
                end
            else
                if state.DefenseMode.value == "Physical" and state.PhysicalDefenseMode.value == "PetPDT" then
                ...
                elseif state.DefenseMode.value == "Magical" and state.MagicalDefenseMode.value == "PetMDT" then
                    ...
                    idleSet = set_combine(idleSet, sets.TPAxeShield)
                end
            end
        end
        if pet.status ~= "Engaged" then
